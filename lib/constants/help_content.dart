class HelpContent {
  static const accesso = [
    'Accedi con le credenziali fornite dalla segreteria.',
    "L'app conserva l'accesso finche' non esegui il logout.",
  ];

  static const calendario = [
    "Viene presentato l'elenco completo delle gare; l'utente puo' dare la propria disponibilita' premendo sull'apposito pulsante, visibile solo con stato DA GESTIRE o IN PROGRESS.",
    'Se sei stato assegnato ad una gara (tramite la pressione del pulsante o dalla Segreteria), visualizzerai il pulsante "Rimuovimi dalla gara" che ti permette di ritirare la tua disponibilita\' PRIMA che lo status della gara sia COPERTA.',
    'Toccare una gara per vedere i dettagli.',
  ];

  static const designazioni = [
    'Vedi solo le gare in cui sei designato.',
    "La sezione e' divisa in \"Servizi da svolgere\" e \"Servizi conclusi\" (in base allo stato della gara).",
    'Tocca una gara per aprire i dettagli.',
  ];

  static const rapportini = [
    "La Segreteria puo' compilare rapportini per tutte le gare.",
    "Gli altri utenti possono compilare rapportini solo se sono DSC della gara e lo stato e' DESIGNAZIONE INVIATA o GARA COMPLETATA.",
    "Dal menu a tendina, selezionare la gara della quale si intende compilare il rapportino. Le informazioni della gara vengono compilate automaticamente, cosi' come l'elenco dei cronometristi designati. In caso di gare di piu' giorni, il sistema propone in automatico l'elenco dei cronometristi designati per ogni giornata.",
    "Il rapportino viene generato in formato PDF dopo la pressione dell'apposito pulsante, e puo' essere inviato alla segreteria via email oppure Whatsapp.",
  ];

  static const archivio = [
    "Consulta i rapportini gia' inviati e scarica i PDF generati in precedenza.",
  ];

  static const noteUtili = [
    'N/A',
  ];
}
